...
class iNaturalistData(AbstractData):
...
class iNaturalistDataSet(AbstractDataSet[iNaturalistData]):
    def __init__(self, path: str | list[str] = ""):
        ...
        for p in path_:
            ...
            for file in data['data']:
                self.data_.append(iNaturalistData(os.path.join(data['path'],file)))

    def __len__(self) -> int:
        '''Return the number of images in the dataset.'''
        return len(self.data_)
    
    def __getitem__(self, idx: int) -> iNaturalistData:
        '''Return the image at the given index.'''
        return self.data_[idx]
    
    def merge(self, other: 'iNaturalistDataSet') -> None:
        '''Merge the dataset with another dataset.'''
        self.data_.extend(other.data_)

    @staticmethod
    def from_list(data: list[iNaturalistData]) -> 'iNaturalistDataSet':
        '''Initialize the image dataset from a list of image data items.'''
        ret = iNaturalistDataSet()
        ret.data_ = data
        return ret
